...
# - ch9 - dictionary (1)
# - ch12 - recursion (1)
# - 2 assignments (10% each), 2 quizzes (2.5% each), 2 bonus labs (0.5% each)
# - final exam (35%)
...
# - two types:
#   - text files: .txt, .py, for human to read, contents are strings
...
# - File organization
#   - In hierarchal structure (tree structure)
...
#       - convert value to string: str(...)
#       - write a new line: '\n' 
#   2.2 read
#       - content = file_object.read() will read the entire file
#       - line = file_object.readline() will read one line
#           - remove new line: string.rstrip('\n')
# 3. close: file_object.close()
#   - release resources, be sure that the data in buffer will be written
#
...
#   - while linw != '': empty string means the end of file
# - CSV: comma seperated values - standard text file format
